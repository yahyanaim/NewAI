// Enhanced Moroccan News Streaming Edge Function
// Fetches news from additional sources for more frequent updates
...
Deno.serve(async (req) => {
  ...
  try {
    console.log('Starting enhanced Moroccan news fetch...');
    ...
    console.error('Enhanced news streaming error:', error);
    
    return new Response(
      JSON.stringify({
        error: {
          code: 'ENHANCED_NEWS_FAILED',
...
// Existing source functions (unchanged)
...
// Fetch Le360 RSS (frequent updates)
async function fetchLe360RSS(): Promise<any[]> {
  try {
    ...
    console.error('Le360 RSS error:', error);
    return [];
  }
}

// Fetch Challenge RSS (business news)
async function fetchChallengeRSS(): Promise<any[]> {
  try {
    ...
    console.error('Challenge RSS error:', error);
    return [];
  }
}

// Fetch AfricaNews RSS (African news)
async function fetchAfricaNewsRSS(): Promise<any[]> {
  try {
    ...
    console.error('AfricaNews RSS error:', error);
    return [];
  }
}

// Fetch Reuters Morocco RSS
async function fetchReutersMoroccoRSS(): Promise<any[]> {
  try {
    ...
    console.error('Reuters Morocco RSS error:', error);
    return [];
  }
}